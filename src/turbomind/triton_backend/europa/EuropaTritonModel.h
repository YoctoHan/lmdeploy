...
template<typename T>
struct EuropaTritonModel: public AbstractTransformerModel {
    EuropaTritonModel(size_t      tensor_para_size,
                      size_t      pipeline_para_size,
                      int         enable_custom_all_reduce,
                      std::string model_dir);

    ~EuropaTritonModel() = default;
...
private:
    std::unique_ptr<EuropaTritonSharedModelInstance<T>>
    ...
    size_t                           num_layer_;
    size_t                           vocab_size_;
    turbomind::EuropaAttentionParams attn_params_;
    float                            norm_eps_;
    int                              max_batch_size_;
    ...
    std::vector<std::shared_ptr<ft::EuropaWeight<T>>> shared_weights_;

    std::shared_ptr<typename ft::Europa<T>::SharedState> shared_state_;

    // weak_ptr is used so that the instances get released when all strong references are gone
    std::vector<std::weak_ptr<EuropaTritonSharedModelInstance<T>>> shared_instances_;
