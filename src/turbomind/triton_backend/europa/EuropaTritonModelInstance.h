...
template<typename T>
struct EuropaTritonModelInstance: AbstractTransformerModelInstance {

    EuropaTritonModelInstance(std::shared_ptr<EuropaTritonSharedModelInstance<T>>    instance,
                             std::unique_ptr<ft::Allocator<ft::AllocatorType::CUDA>> allocator);
    ~EuropaTritonModelInstance();
...
private:
    const std::shared_ptr<EuropaTritonSharedModelInstance<T>>     instance_;
