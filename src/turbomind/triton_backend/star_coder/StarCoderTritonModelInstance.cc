...
{
    StarCoderTritonModelInstance<T>* model  = reinterpret_cast<StarCoderTritonModelInstance<T>*>(ctx);
    auto                         result = StarCoderTritonModelInstance<T>::convert_outputs(*output_tensors);

    model->stream_cb_(result, model->stream_ctx_);
}

template<typename T>
StarCoderTritonModelInstance<T>::StarCoderTritonModelInstance(
...
template<typename T>
std::unordered_map<std::string, ft::Tensor> StarCoderTritonModelInstance<T>::convert_inputs(
...
StarCoderTritonModelInstance<T>::convert_outputs(const std::unordered_map<std::string, ft::Tensor>& output_tensors)
{
    TM_LOG_DEBUG(__PRETTY_FUNCTION__);
...
StarCoderTritonModelInstance<T>::forward(std::shared_ptr<std::vector<triton::Tensor>> input_tensors)
{
    ft::FT_CHECK(false);
    return nullptr;
}
...
template<typename T>
std::shared_ptr<std::unordered_map<std::string, triton::Tensor>>
StarCoderTritonModelInstance<T>::forward(std::shared_ptr<std::unordered_map<std::string, triton::Tensor>> input_tensors)
{
    ft::FT_CHECK(false);
    return nullptr;
}
                                     ...
                                     ft::AbstractInstanceComm*                                        instance_comm)
{
    ...
    const size_t vocab_size = instance_->star_coder->vocab_size();
    ...
    try {
        ...
        instance_->star_coder->forward(&output_tensors, &ft_input_tensors, {instance_comm, callback});
...
template<typename T>
void StarCoderTritonModelInstance<T>::allocateBuffer(const size_t request_batch_size,
                                                 ...
                                                 const bool   is_return_logits)
{
    ...
    if (is_return_logits) {
        d_output_logits_ = (float*)allocator_->reMalloc(
            d_output_logits_, sizeof(float) * request_batch_size * max_input_len * instance_->star_coder->vocab_size(), false);
    }
}

template<typename T>
void StarCoderTritonModelInstance<T>::freeBuffer()
{
...
template struct StarCoderTritonModelInstance<float>;
template struct StarCoderTritonModelInstance<half>;
