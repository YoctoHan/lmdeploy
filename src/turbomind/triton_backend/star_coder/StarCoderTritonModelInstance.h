...
struct StarCoderTritonSharedModelInstance {
    std::unique_ptr<ft::StarCoder<T>>                       star_coder;
    std::shared_ptr<ft::StarCoderWeight<T>>                 star_coder_weight;
...
template<typename T>
struct StarCoderTritonModelInstance: AbstractTransformerModelInstance {

    StarCoderTritonModelInstance(std::shared_ptr<StarCoderTritonSharedModelInstance<T>>      instance,
                             std::unique_ptr<ft::Allocator<ft::AllocatorType::CUDA>> allocator);
    ~StarCoderTritonModelInstance();
...
private:
    const std::shared_ptr<StarCoderTritonSharedModelInstance<T>>      instance_;
