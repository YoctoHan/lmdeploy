...
#include "src/turbomind/models/star_coder/StarCoderInstanceComm.h"
...
std::shared_ptr<AbstractTransformerModel> AbstractTransformerModel::createStarCoderModel(std::string inifile)
{
    ...
    if (data_type == "half" || data_type == "fp16") {
        return std::make_shared<StarCoderTritonModel<half>>(
        ...
        return std::make_shared<StarCoderTritonModel<float>>(
            reader.GetInteger("ft_instance_hyperparameter", "tensor_para_size"),
            reader.GetInteger("ft_instance_hyperparameter", "pipeline_para_size"),
            reader.GetInteger("ft_instance_hyperparameter", "enable_custom_all_reduce", 0),
            model_dir);
...
template<typename T>
void StarCoderTritonModel<T>::handleMissingParams()
{
    if (kv_head_num_ == 0) {
        kv_head_num_ = head_num_;
        TM_LOG_WARNING("[StarCoderTritonModel] `kv_head_num` is not set, default to `head_num` (%d).", (int)kv_head_num_);
    }

    if (!max_batch_size_) {
        max_batch_size_ = 32;
        TM_LOG_WARNING("[StarCoderTritonModel] `max_batch_size` is not set, default to %d.", (int)max_batch_size_);
    }

    if (!session_len_) {
        session_len_ = 2160;
        TM_LOG_WARNING("[StarCoderTritonModel] `session_len` is not set, default to %d.", (int)session_len_);
    }

    if (!max_context_token_num_) {
        max_context_token_num_ = (int)std::sqrt(max_batch_size_);
        TM_LOG_WARNING("[StarCoderTritonModel] `max_context_token_num` is not set, default to %d.",
                       (int)max_context_token_num_);
    }

    if (!step_length_) {
        step_length_ = 1;
        TM_LOG_WARNING("[StarCoderTritonModel] `step_length` is not set, default to %d.", (int)step_length_);
    }

    if (!cache_max_entry_count_) {
        cache_max_entry_count_ = 32;
        TM_LOG_WARNING("[StarCoderTritonModel] `cache_max_entry_count` is not set, default to %d.",
                       (int)cache_max_entry_count_);
    }

    if (!cache_chunk_size_) {
        cache_chunk_size_ = cache_max_entry_count_;
        TM_LOG_WARNING("[StarCoderTritonModel] `cache_chunk_size` is not set, default to %d.", (int)cache_chunk_size_);
    }
}

template<typename T>
StarCoderTritonModel<T>::StarCoderTritonModel(size_t      tensor_para_size,
                                      size_t      pipeline_para_size,
                                      int         enable_custom_all_reduce,
                                      std::string model_dir):
    tensor_para_size_(tensor_para_size),
    pipeline_para_size_(pipeline_para_size),
    shared_weights_(std::vector<std::shared_ptr<ft::StarCoderWeight<T>>>(ft::getDeviceCount())),
    enable_custom_all_reduce_(enable_custom_all_reduce)
{
    ...
    model_name_            = reader.Get("star_coder", "model_name");
    head_num_              = reader.GetInteger("star_coder", "head_num");
    kv_head_num_           = reader.GetInteger("star_coder", "kv_head_num", 0);
    size_per_head_         = reader.GetInteger("star_coder", "size_per_head");
    inter_size_            = reader.GetInteger("star_coder", "inter_size");
    num_layer_             = reader.GetInteger("star_coder", "num_layer");
    vocab_size_            = reader.GetInteger("star_coder", "vocab_size");
    norm_eps_              = reader.GetFloat("star_coder", "norm_eps");
    start_id_              = reader.GetInteger("star_coder", "start_id");
    end_id_                = reader.GetInteger("star_coder", "end_id");
    max_batch_size_        = reader.GetInteger("star_coder", "max_batch_size", 0);
    max_context_token_num_ = reader.GetInteger("star_coder", "max_context_token_num", 0);
    session_len_           = reader.GetInteger("star_coder", "session_len", 0);
    step_length_           = reader.GetInteger("star_coder", "step_length", 0);
    cache_max_entry_count_ = reader.GetInteger("star_coder", "cache_max_entry_count", 0);
    use_context_fmha_      = reader.GetInteger("star_coder", "use_context_fmha", 1);
    cache_chunk_size_      = reader.GetInteger("star_coder", "cache_chunk_size", 0);
    attn_bias_             = reader.GetInteger("star_coder", "attn_bias", 0);
    quant_policy_          = reader.GetInteger("star_coder", "quant_policy", 0);
    group_size_            = reader.GetInteger("star_coder", "group_size", 0);

    attn_params_.rotray_embedding_dim    = reader.GetInteger("star_coder", "rotary_embedding");
    attn_params_.rotary_embedding_base   = reader.GetFloat("star_coder", "rope_theta", 10000.0f);
    attn_params_.max_position_embeddings = reader.GetInteger("star_coder", "max_position_embeddings", 0);
    attn_params_.use_dynamic_ntk         = reader.GetInteger("star_coder", "use_dynamic_ntk", 0);
    attn_params_.use_logn_attn           = reader.GetInteger("star_coder", "use_logn_attn", 0);
    ...
    shared_state_          = std::make_shared<typename ft::StarCoder<T>::SharedState>();
    ...
    const std::string weight_type_str = reader.Get("star_coder", "weight_type");
...
template<typename T>
std::unique_ptr<StarCoderTritonSharedModelInstance<T>> StarCoderTritonModel<T>::createSharedModelInstance(
    int                                                               device_id,
    int                                                               rank,
    ...
    std::shared_ptr<ft::AbstractCustomComm>                           custom_all_reduce_comm)
{
    ...
    auto star_coder = std::make_unique<ft::StarCoder<T>>(head_num_,
                                                  kv_head_num_,
                                                  size_per_head_,
                                                  inter_size_,
                                                  num_layer_,
    ...
    return std::make_unique<StarCoderTritonSharedModelInstance<T>>(
        StarCoderTritonSharedModelInstance<T>{std::move(star_coder),
...
StarCoderTritonModel<T>::createModelInstance(int                                                               device_id,
                                         int                                                               rank,
                                         cudaStream_t                                                      stream,
                                         std::pair<std::vector<ft::NcclParam>, std::vector<ft::NcclParam>> nccl_params,
                                         std::shared_ptr<ft::AbstractCustomComm> custom_all_reduce_comm)
{
    ...
    std::shared_ptr<StarCoderTritonSharedModelInstance<T>> instance;
    {
        ...
        if (!instance) {
            ...
            instance->star_coder->setFfiLock(ffi_lock_);
    ...
    return std::make_unique<StarCoderTritonModelInstance<T>>(instance, std::move(allocator));
}

template<typename T>
void StarCoderTritonModel<T>::createSharedWeights(int device_id, int rank)
{
    ft::check_cuda_error(cudaSetDevice(device_id));
    const int tensor_para_rank   = rank % tensor_para_size_;
    const int pipeline_para_rank = rank / tensor_para_size_;
    ft::FT_CHECK(pipeline_para_size_ == 1 && pipeline_para_rank == 0);
    shared_weights_[device_id] = std::make_shared<ft::StarCoderWeight<T>>(head_num_,
                                                                      kv_head_num_,
                                                                      size_per_head_,
                                                                      inter_size_,
                                                                      vocab_size_,
...
template<typename T>
std::string StarCoderTritonModel<T>::toString()
{
    std::stringstream ss;
    ss << "Model: "
...
template<typename T>
void StarCoderTritonModel<T>::createCustomComms(
...
StarCoderTritonModel<T>::createNcclParams(const int node_id, const int device_id_start, const bool multi_node)
{
    const auto device_count     = ft::getDeviceCount();
...
template<typename T>
std::unique_ptr<ft::AbstractInstanceComm> StarCoderTritonModel<T>::createInstanceComm(int size)
{
    return std::make_unique<ft::StarCoderInstanceComm>(size);
}

template<typename T>
int StarCoderTritonModel<T>::getTensorParaSize()
{
    return tensor_para_size_;
}

template<typename T>
int StarCoderTritonModel<T>::getPipelineParaSize()
{
    return pipeline_para_size_;
}

template struct StarCoderTritonModel<float>;
template struct StarCoderTritonModel<half>;
