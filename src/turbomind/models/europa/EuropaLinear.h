...
public:
    ...
    EuropaLinear(cublasMMWrapper* cublas_wrapper, cudaStream_t stream): cublas_wrapper_(cublas_wrapper), stream_(stream)
    {
    }

    void
    forward(T* output_data, const T* input_data, int batch_size, const EuropaDenseWeight<T>& weight, Type type = kGemm)
    {
        switch (weight.type) {
...
private:
    void forwardFp(T* output_data, const T* input_data, int batch_size, const EuropaDenseWeight<T>& weight, Type type)
    {
        FT_CHECK(type == kGemm);
    ...
    void forwardInt4(T* output_data, const T* input_data, int batch_size, const EuropaDenseWeight<T>& weight, Type type)
    {
