...
template<typename T>
__global__ void rootMeanSquareNormEuropa(T* out, const T* input, const T* scale, float eps, int m, int n)
{
...
template<>
__global__ void rootMeanSquareNormEuropa(float* out, const float* input, const float* scale, float eps, int m, int n)
{
    __shared__ float s_inv_mean;
    float            mean = 0.f;
...
void invokeRootMeanSquareNorm(T* out, const T* input, const T* scale, float eps, int m, int n, cudaStream_t stream)
{
    ...
    rootMeanSquareNormEuropa<<<grid, block, 0, stream>>>(out, input, scale, eps, m, n);
}
...
__device__ T saturate_cast_europa(T0 x)
{
    return x;
}

template<>
__device__ half saturate_cast_europa<half, float>(float x)
{
