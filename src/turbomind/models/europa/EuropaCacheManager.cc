...
#include "src/turbomind/models/europa/EuropaCacheManager.h"
#include "src/turbomind/utils/cuda_utils.h"
#include "src/turbomind/utils/logger.h"

namespace turbomind {

EuropaCacheManager::~EuropaCacheManager()
{
...
void* EuropaCacheManager::allocate(bool is_preallocte)
{
    if (rank_ == 0) {
        TM_LOG_INFO("[EuropaCacheManager][allocate]");
    }
    ...
    if (!device_free_.empty()) {
        ...
        if (rank_ == 0) {
            TM_LOG_INFO("[EuropaCacheManager][allocate] free = %d", (int)device_free_.size());
        }
    }
        ...
        if (rank_ == 0) {
            TM_LOG_INFO("[EuropaCacheManager][allocate] malloc %d", (int)alloc_count);
        }
        ...
        if (rank_ == 0) {
            TM_LOG_INFO("[EuropaCacheManager][allocate] count = %d", entry_count_);
        }
...
auto EuropaCacheManager::create(uint64_t id, cudaStream_t stream) -> Sequence
{
    if (rank_ == 0) {
        TM_LOG_INFO("[EuropaCacheManager][create] %ld", (long)id);
    }

    for (const auto& e : device_cache_) {
        if (e.id == id) {
            if (rank_ == 0) {
                TM_LOG_WARNING("[EuropaCacheManager][create] Removing conflicting id %ld", (long)id);
            }
            erase(id);
        }
    }
...
auto EuropaCacheManager::getEntryOrThrow(uint64_t id) -> std::vector<Sequence>::iterator
{
    auto pred = [&](const Sequence& s) { return s.id == id; };
    auto it   = std::find_if(device_cache_.begin(), device_cache_.end(), pred);
    if (it == device_cache_.end()) {
        TM_LOG_ERROR("[EuropaCacheManager] %ld not found.\n", (long)id);
        FT_CHECK(0);
    }
    return it;
}

auto EuropaCacheManager::fetch(uint64_t id, cudaStream_t stream) -> Sequence
{
    if (rank_ == 0) {
        TM_LOG_INFO("[EuropaCacheManager][fetch] %ld", (long)id);
    }
...
void EuropaCacheManager::update(const Sequence& seq, cudaStream_t stream)
{
    if (rank_ == 0) {
        TM_LOG_INFO("[EuropaCacheManager][update] %ld", (long)seq.id);
    }
...
void EuropaCacheManager::erase(uint64_t id)
{
    if (rank_ == 0) {
        TM_LOG_INFO("[EuropaCacheManager][erase] %ld", (long)id);
    }
    ...
    if (entry->k_cache) {
        ...
        if (rank_ == 0) {
            TM_LOG_INFO("[EuropaCacheManager][erase] free = %d", (int)device_free_.size());
        }
    }
    device_cache_.erase(entry);
}

void* EuropaCacheManager::evict()
{
    ...
    if (rank_ == 0) {
        TM_LOG_INFO("[EuropaCacheManager][evict] %ld", (long)it->id);
    }
...
bool EuropaCacheManager::contains(uint64_t id) const noexcept
{
    auto pred = [&](const Sequence& s) { return s.id == id; };
    auto it   = std::find_if(device_cache_.begin(), device_cache_.end(), pred);
