...
template<typename T>
class EuropaDecoderSelfAttentionLayer {
public:
    void freeBuffer();
    void allocateBuffer(size_t batch_size, int key_len, int max_memory_len);

    EuropaDecoderSelfAttentionLayer(size_t                      head_num,
                                   size_t                      kv_head_num,
    ...
    ~EuropaDecoderSelfAttentionLayer()
    {
        freeBuffer();
    }

    void forward(TensorMap* output_tensors, const TensorMap* input_tensors, const EuropaAttentionWeight<T>* weights);
...
private:
    ...
    const EuropaAttentionParams& params_;
