...
#include "src/turbomind/models/europa/EuropaFfnLayer.h"

namespace turbomind {

template<typename T>
void EuropaFfnLayer<T>::forward(TensorMap* output_tensors,
                          ...
                          const EuropaFfnWeight<T>* ffn_weights)
{
    ...
    invokeEuropaAddBiasActivation(m, ffn_weights->dense_h_to_4h.bias);
...
template<typename T>
EuropaFfnLayer<T>::EuropaFfnLayer(size_t head_num,
...
template<typename T>
EuropaFfnLayer<T>::~EuropaFfnLayer()
{
    TM_LOG_DEBUG(__PRETTY_FUNCTION__);
...
template<typename T>
void EuropaFfnLayer<T>::allocateBuffer()
{
    TM_LOG_DEBUG(__PRETTY_FUNCTION__);
...
template<typename T>
void EuropaFfnLayer<T>::allocateBuffer(size_t token_num)
{
    TM_LOG_DEBUG(__PRETTY_FUNCTION__);
...
template<typename T>
void EuropaFfnLayer<T>::freeBuffer()
{
    TM_LOG_DEBUG(__PRETTY_FUNCTION__);
...
template<typename T>
bool EuropaFfnLayer<T>::isValidTokenNum(size_t token_num)
{
...
template class EuropaFfnLayer<float>;
template class EuropaFfnLayer<half>;
#ifdef ENABLE_BF16
template class EuropaFfnLayer<__nv_bfloat16>;
#endif

template<typename T>
EuropaGeluFfnLayer<T>::EuropaGeluFfnLayer(size_t head_num,
                                          size_t size_per_head,
                                          size_t inter_size,
                                          cudaStream_t stream,
                                          cublasMMWrapper* cublas_wrapper,
...
template<typename T>
void EuropaGeluFfnLayer<T>::invokeEuropaAddBiasActivation(const int m, const T* bias)
{
    invokeAddBiasGeluV2<T>(input_buf_, bias, nullptr, nullptr, nullptr, m, m, 6144 * 4, stream_);
}

template class EuropaGeluFfnLayer<float>;
template class EuropaGeluFfnLayer<half>;
#ifdef ENABLE_BF16
template class EuropaGeluFfnLayer<__nv_bfloat16>;
#endif
