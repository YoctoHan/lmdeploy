...
template<typename T>
EuropaDecoderLayerWeight<T>::EuropaDecoderLayerWeight(size_t     head_num,
                                                      size_t     kv_head_num,
...
template<typename T>
void freeWeights(EuropaDenseWeight<T>& weights)
{
...
template<typename T>
void mallocWeights(EuropaDenseWeight<T>& weights, bool bias)
{
    if (bias) {
...
template<typename T>
void loadWeights(EuropaDenseWeight<T>& w,
                 std::string           prefix,
                 int                   rank,
...
template<typename T>
void EuropaDecoderLayerWeight<T>::mallocWeights()
{
...
template<typename T>
EuropaDecoderLayerWeight<T>::~EuropaDecoderLayerWeight()
{
...
template<typename T>
void EuropaDecoderLayerWeight<T>::loadModel(std::string dir_path, FtCudaDataType model_file_type)
{
    ...
    loadWeights(self_attn_weights.qkv,
                dir_path + ".attention.query_key_value",
                tensor_para_rank_,
                type,
                tensor_para_size_,
                1,
...
template struct EuropaDecoderLayerWeight<float>;
template struct EuropaDecoderLayerWeight<half>;
