...
#include "src/turbomind/models/europa/EuropaBatch.h"
#include "src/turbomind/kernels/decoding_kernels.h"
#include "src/turbomind/macro.h"
#include "src/turbomind/models/llama/LlamaNcclGuard.h"
#include "src/turbomind/models/europa/Europa.h"
    ...
    auto handle_conflict_or_invalid = [this, &occurrence, &invalidate](std::vector<std::shared_ptr<Request>>& rs,
        ...
        for (auto& r : rs) {
            if (r) {
                ...
                else if (!r->start_flag && !europa_->kv_cache_mgr_->contains(r->id)) {
                    ec = Request::kInvalid;
                }
...
template<typename T>
void EuropaBatch<T>::handleStopRequests(const std::vector<std::shared_ptr<Request>>& requests)
{
    for (const auto& r : requests) {
        ...
        if (ec && r->end_flag) {
            ec = 0;
            europa_->kv_cache_mgr_->erase(r->id);
        }
...
template<typename T>
void EuropaBatch<T>::allocateBuffer(size_t batch_size, size_t session_len)
{
    TM_LOG_DEBUG(__PRETTY_FUNCTION__);
    const size_t batchxbeam = batch_size;

    const size_t hidden_units = europa_->hidden_units_;
    const size_t vocab_size   = europa_->vocab_size_padded_;
...
template<typename T>
void EuropaBatch<T>::allocatePersistantBuffer(size_t max_batch_size)
{
    ...
    {
        NcclGuard barrier(europa_->tensor_para_, stream_, true);
        h_input_ids_buf_ =
...
template<typename T>
EuropaBatch<T>::EuropaBatch(int max_batch_size, int max_context_token_num, int session_len, Europa<T>* europa):
    max_batch_size_(max_batch_size),
    max_context_token_num_(max_context_token_num),
    session_len_(session_len),
    rank_(europa->tensor_para_.rank_),
    debug_(europa->debug_),
    europa_(europa),
    data_type_(getTensorType<T>())
{
    stream_         = europa_->stream_;
    allocator_      = europa_->allocator_;
    cublas_wrapper_ = europa_->cublas_wrapper_;
...
template<typename T>
void EuropaBatch<T>::initializeSampling(int infer_request_count)
{
    TensorMap inputs;
    ...
    europa_->dynamic_decode_layer_->setup(batch_size_, 1, &inputs_);
    ...
    for (int i = 0; i < batch_size_; ++i) {
        ...
        if (i < batch_size_ - infer_request_count || !requests_[i]->inputs[rank_].isExist("random_seed")) {
            check_cuda_error(cudaMemcpyAsync(europa_->dynamic_decode_layer_->topk_curandstate_buf() + i,
                                             (curandState_t*)topk_curandstate_buf_ + i,
                                             sizeof(curandState_t),
                                             cudaMemcpyDefault,
                                             stream_));
            check_cuda_error(cudaMemcpyAsync(europa_->dynamic_decode_layer_->topp_curandstate_buf() + i,
    ...
    handleOptArg(&inputs_, "end_id", end_ids_buf_, europa_->end_id_, batch_size_);
    cudaStreamSynchronize(0);
}

template<typename T>
void EuropaBatch<T>::initializeGeneration()
{
...
template<typename T>
void EuropaBatch<T>::initialize(const std::vector<std::shared_ptr<Request>>& infer_requests)
{
    ...
    for (int i = 0; i < infer_request_count; ++i) {
        ...
        EuropaCacheManager::Sequence seq{};
        if (r.start_flag) {
            seq = europa_->kv_cache_mgr_->create(r.id, stream_);
        }
        else {
            seq = europa_->kv_cache_mgr_->fetch(r.id, stream_);
        }
        ...
        if (seq.token_ids.empty() && actual_input_len == 0) {
            seq.token_ids.push_back(europa_->start_id_);
            seq.cache_len    = 0;
    ...
    if (europa_->tensor_para_.rank_ == 0) {
...
template<typename T>
void EuropaBatch<T>::contextDecode()
{

    // printf("\n -*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*");
    // printf("\n -*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*- EuropaBatch<T>::contextDecode -*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-* \n");    
                                            ...
                                            const std::vector<int>& lengths)
{
    ...
    if (context_logits_buf_ == nullptr) {
        NcclGuard guard(europa_->tensor_para_, stream_, true);
        context_logits_buf_ =
            (float*)allocator_->malloc(sizeof(float) * europa_->vocab_size_padded_ * max_context_token_num_);
        const auto tp = europa_->tensor_para_.world_size_;
        if (tp > 1) {
            FT_CHECK(europa_->vocab_size_padded_ % tp == 0);
            const auto local_vocab_size = europa_->vocab_size_padded_ / tp;
            local_context_logits_buf_ =
                (float*)allocator_->malloc(sizeof(float) * local_vocab_size * max_context_token_num_);
        }
    }

    europa_->postDecodeEmbedding(context_logits_buf_, local_context_logits_buf_, context_decoder_output, num_token);
    ...
    for (int k = 0; k < indices.size(); ++k) {
        if (output_logits[k]) {
            check_cuda_error(cudaMemcpyAsync(output_logits[k],
                                             logits,
                                             sizeof(float) * europa_->vocab_size_ * lengths[k],
                                             cudaMemcpyDefault,
                                             stream_));
        }
        logits += europa_->vocab_size_padded_ * lengths[k];
    }
}

template<typename T>
void EuropaBatch<T>::finish()
{
...
template<typename T>
void EuropaBatch<T>::synchronize()
{
    // compact
    int idx = 0;
    for (int i = 0; i < batch_size_; ++i) {
        if (requests_[i]) {
            ...
            check_cuda_error(cudaMemcpyAsync((curandState_t*)topk_curandstate_buf_ + idx,
                                             europa_->dynamic_decode_layer_->topk_curandstate_buf() + i,
                                             sizeof(curandState_t),
                                             cudaMemcpyDefault,
                                             stream_));
            check_cuda_error(cudaMemcpyAsync((curandState_t*)topp_curandstate_buf_ + idx,
                                             europa_->dynamic_decode_layer_->topp_curandstate_buf() + i,
                                             sizeof(curandState_t),
                                             cudaMemcpyDefault,
                                             stream_));
...
template<typename T>
void EuropaBatch<T>::setOutputTensors(int max_gen_step)
{
...
template<typename T>
void EuropaBatch<T>::finishRequest(int index, bool force_end)
{
    if (rank_ == 0) {
    ...
    if (requests_[index]->end_flag || force_end) {
        europa_->kv_cache_mgr_->erase(requests_[index]->id);
    }
    else {
        ...
        check_cuda_error(cudaMemcpyAsync(seq.random_state_.data(),
                                         europa_->dynamic_decode_layer_->topk_curandstate_buf() + index,
                                         sizeof(curandState_t),
                                         cudaMemcpyDefault,
                                         stream_));
        check_cuda_error(cudaMemcpyAsync(seq.random_state_.data() + sizeof(curandState_t),
                                         europa_->dynamic_decode_layer_->topp_curandstate_buf() + index,
                                         sizeof(curandState_t),
                                         cudaMemcpyDefault,
                                         stream_));
        ...
        europa_->kv_cache_mgr_->update(cached_seq_[index], stream_);
    }
...
template class EuropaBatch<half>;
template class EuropaBatch<float>;
