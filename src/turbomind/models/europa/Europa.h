...
private:
    ...
    void
    initialize(const EuropaAttentionParams& attn_params, size_t kv_head_num, bool use_context_fmha, int quant_policy);
...
private:
    ...
    std::unique_ptr<EuropaCacheManager> kv_cache_mgr_;

    EuropaWeight<T>*            weights_{};
    EuropaDecoder<T>*           decoder_{};
    EuropaContextDecoder<T>*    context_decoder_{};
    DynamicDecodeLayer<float>*  dynamic_decode_layer_{};

    const int                     step_length_;
    EuropaBatch<T>                batch_;
