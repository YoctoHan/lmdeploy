...
template<typename T>
Europa<T>::Europa(size_t                           head_num,
                  size_t                           kv_head_num,
    ...
    shared_state_(shared_state)
{
    ...
    kv_cache_mgr_ = std::make_unique<EuropaCacheManager>(num_layer_,
                                                         local_kv_head_num,
                                                         size_per_head_,
                                                         session_len,
                                                         elem_bits,
...
template<typename T>
void Europa<T>::initialize(const EuropaAttentionParams& attn_params,
                           size_t                       kv_head_num,
                           bool                         use_context_fmha,
                           int                          quant_policy)
{
    ...
    context_decoder_ = new EuropaContextDecoder<T>(head_num_,
                                                   kv_head_num,
                                                   size_per_head_,
                                                   inter_size_,
                                                   num_layer_,
    ...
    decoder_ = new EuropaDecoder<T>(head_num_,
                                    kv_head_num,
                                    size_per_head_,
                                    inter_size_,
                                    num_layer_,
...
template<typename T>
void Europa<T>::embeddingLookup(T* embeddings, const int* token_ids_buf, int batch_size, int step)
{
    TM_LOG_DEBUG(__PRETTY_FUNCTION__);
...
template<typename T>
void Europa<T>::contextDecode(T*         deocder_output,
...
template<typename T>
void Europa<T>::decoderForward(T*         decoder_output,
                               uintptr_t* k_cache_ptr,
                               uintptr_t* v_cache_ptr,
                               T*         decoder_input,
...
template<typename T>
void Europa<T>::postDecodeEmbedding(float* logits, float* local_logits, const T* decoder_output, int batch_size)
{
    TM_LOG_DEBUG(__PRETTY_FUNCTION__);
...
template<typename T>
void Europa<T>::dynamicDecode(int*            token_ids,
                              bool*           finished,
                              int*            sequence_length,
                              bool*           should_stop,
                              TensorMap*      inputs,
...
template<typename T>
void Europa<T>::internalThreadEntry(int device_id)
{
...
void Europa<T>::start()
{
    ...
    internal_thread_ = std::thread(&Europa<T>::internalThreadEntry, this, device_id);
}
...
template<typename T>
void Europa<T>::forward(std::unordered_map<std::string, Tensor>*       outputs,
...
template class Europa<half>;
template class Europa<float>;
