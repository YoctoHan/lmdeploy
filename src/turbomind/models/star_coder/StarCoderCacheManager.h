...
class StarCoderCacheManager {
public:
    StarCoderCacheManager(size_t      layer_num,
                      size_t      head_num,
    ...
    {
        if (rank == 0) {
            TM_LOG_INFO("[StarCoderCacheManager] max_entry_count = %d", (int)max_entry_count_);
            TM_LOG_INFO("[StarCoderCacheManager] chunk_size = %d", (int)chunk_size_);
        }
        allocate(true);
    }

    ~StarCoderCacheManager();
