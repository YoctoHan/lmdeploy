...
#include "src/turbomind/models/star_coder/StarCoderDecoder.h"
#include "src/turbomind/macro.h"
#include "src/turbomind/models/star_coder/star_coder_decoder_kernels.h"
...
template<typename T>
StarCoderDecoder<T>::StarCoderDecoder(size_t                      head_num,
                              size_t                      kv_head_num,
...
template<typename T>
StarCoderDecoder<T>::~StarCoderDecoder()
{
    TM_LOG_DEBUG(__PRETTY_FUNCTION__);
...
template<typename T>
void StarCoderDecoder<T>::initialize(const StarCoderAttentionParams& attn_params, size_t kv_head_num, int quant_policy)
{
    TM_LOG_DEBUG(__PRETTY_FUNCTION__);

    self_attention_layer_ = new StarCoderDecoderSelfAttentionLayer<T>(head_num_,
                                                                  kv_head_num,
                                                                  size_per_head_,
                                                                  attn_params,
                                                                  tensor_para_,
...
template<typename T>
void StarCoderDecoder<T>::allocateBuffer()
{
    FT_CHECK(false);
}

template<typename T>
void StarCoderDecoder<T>::allocateBuffer(size_t batch_size)
{
    TM_LOG_DEBUG(__PRETTY_FUNCTION__);
...
template<typename T>
void StarCoderDecoder<T>::freeBuffer()
{
    TM_LOG_DEBUG(__PRETTY_FUNCTION__);
...
template<typename T>
void StarCoderDecoder<T>::forwardSelfAttn(const StarCoderDecoder::Session&                   sess,
                                      T*                                             attn_io,
...
template<typename T>
void StarCoderDecoder<T>::forwardFfn(const StarCoderDecoder::Session& sess, T* ffn_io, size_t layer)
{
...
template<typename T>
void StarCoderDecoder<T>::forward(std::vector<Tensor>*                            output_tensors,
...
template<typename T>
void StarCoderDecoder<T>::forward(std::unordered_map<std::string, Tensor>*        output_tensors,
                              const std::unordered_map<std::string, Tensor>*  input_tensors,
                              const std::vector<StarCoderDecoderLayerWeight<T>*>* decoder_layer_weights)
{
    TM_LOG_DEBUG(__PRETTY_FUNCTION__);
...
template class StarCoderDecoder<half>;
template class StarCoderDecoder<float>;
