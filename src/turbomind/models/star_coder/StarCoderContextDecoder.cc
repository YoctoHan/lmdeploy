...
template<typename T>
void StarCoderContextDecoder<T>::allocateBuffer()
{
    FT_CHECK(false);
}

template<typename T>
void StarCoderContextDecoder<T>::allocateBuffer(size_t batch_size, size_t num_token, size_t max_q_len, size_t max_kv_len)
{
...
template<typename T>
void StarCoderContextDecoder<T>::freeBuffer()
{
    TM_LOG_DEBUG(__PRETTY_FUNCTION__);
...
template class StarCoderContextDecoder<float>;
template class StarCoderContextDecoder<half>;
