...
template<typename T>
__global__ void rootMeanSquareNormStarCoder(T* out, const T* input, const T* scale, float eps, int m, int n)
{
...
template<>
__global__ void rootMeanSquareNormStarCoder(float* out, const float* input, const float* scale, float eps, int m, int n)
{
    __shared__ float s_inv_mean;
    float            mean = 0.f;
...
void invokeRootMeanSquareNorm(T* out, const T* input, const T* scale, float eps, int m, int n, cudaStream_t stream)
{
    ...
    rootMeanSquareNormStarCoder<<<grid, block, 0, stream>>>(out, input, scale, eps, m, n);
}
...
__device__ T saturate_cast_star_coder(T0 x)
{
    return x;
}

template<>
__device__ half saturate_cast_star_coder<half, float>(float x)
{
