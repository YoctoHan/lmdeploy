...
#include "src/turbomind/models/star_coder/StarCoderBatch.h"
#include "src/turbomind/kernels/decoding_kernels.h"
#include "src/turbomind/macro.h"
#include "src/turbomind/models/llama/LlamaNcclGuard.h"
#include "src/turbomind/models/star_coder/StarCoder.h"
...
template<typename T>
void StarCoderBatch<T>::verifyRequests(std::vector<std::shared_ptr<Request>>& stop_reqs,
    ...
    auto handle_conflict_or_invalid = [this, &occurrence, &invalidate](std::vector<std::shared_ptr<Request>>& rs,
        ...
        for (auto& r : rs) {
            if (r) {
                ...
                else if (!r->start_flag && !star_coder_->kv_cache_mgr_->contains(r->id)) {
                    ec = Request::kInvalid;
                }
...
template<typename T>
void StarCoderBatch<T>::handleStopRequests(const std::vector<std::shared_ptr<Request>>& requests)
{
    for (const auto& r : requests) {
        ...
        if (ec && r->end_flag) {
            ec = 0;
            star_coder_->kv_cache_mgr_->erase(r->id);
        }
...
template<typename T>
void StarCoderBatch<T>::allocateBuffer(size_t batch_size, size_t session_len)
{
    TM_LOG_DEBUG(__PRETTY_FUNCTION__);
    const size_t batchxbeam = batch_size;

    const size_t hidden_units = star_coder_->hidden_units_;
    const size_t vocab_size   = star_coder_->vocab_size_padded_;
...
void StarCoderBatch<T>::allocatePersistantBuffer(size_t max_batch_size)
{
    ...
    {
        NcclGuard barrier(star_coder_->tensor_para_, stream_, true);
        h_input_ids_buf_ =
...
template<typename T>
void StarCoderBatch<T>::freeBuffer()
{
    TM_LOG_DEBUG(__PRETTY_FUNCTION__);
...
template<typename T>
StarCoderBatch<T>::StarCoderBatch(int max_batch_size, int max_context_token_num, int session_len, StarCoder<T>* star_coder):
    max_batch_size_(max_batch_size),
    max_context_token_num_(max_context_token_num),
    session_len_(session_len),
    rank_(star_coder->tensor_para_.rank_),
    debug_(star_coder->debug_),
    star_coder_(star_coder),
    data_type_(getTensorType<T>())
{
    stream_         = star_coder_->stream_;
    allocator_      = star_coder_->allocator_;
    cublas_wrapper_ = star_coder_->cublas_wrapper_;
...
template<typename T>
void StarCoderBatch<T>::initializeSampling(int infer_request_count)
{
    TensorMap inputs;
    ...
    star_coder_->dynamic_decode_layer_->setup(batch_size_, 1, &inputs_);
    ...
    for (int i = 0; i < batch_size_; ++i) {
        ...
        if (i < batch_size_ - infer_request_count || !requests_[i]->inputs[rank_].isExist("random_seed")) {
            check_cuda_error(cudaMemcpyAsync(star_coder_->dynamic_decode_layer_->topk_curandstate_buf() + i,
                                             (curandState_t*)topk_curandstate_buf_ + i,
                                             sizeof(curandState_t),
                                             cudaMemcpyDefault,
                                             stream_));
            check_cuda_error(cudaMemcpyAsync(star_coder_->dynamic_decode_layer_->topp_curandstate_buf() + i,
    ...
    handleOptArg(&inputs_, "end_id", end_ids_buf_, star_coder_->end_id_, batch_size_);
    cudaStreamSynchronize(0);
}

template<typename T>
void StarCoderBatch<T>::initializeGeneration()
{
...
template<typename T>
bool StarCoderBatch<T>::generate()
{
...
template<typename T>
void StarCoderBatch<T>::initialize(const std::vector<std::shared_ptr<Request>>& infer_requests)
{
    ...
    for (int i = 0; i < infer_request_count; ++i) {
        ...
        StarCoderCacheManager::Sequence seq{};
        if (r.start_flag) {
            seq = star_coder_->kv_cache_mgr_->create(r.id, stream_);
        }
        else {
            seq = star_coder_->kv_cache_mgr_->fetch(r.id, stream_);
        }
        ...
        if (seq.token_ids.empty() && actual_input_len == 0) {
            seq.token_ids.push_back(star_coder_->start_id_);
            seq.cache_len    = 0;
    ...
    if (star_coder_->tensor_para_.rank_ == 0) {
    ...
    if (base >= 0) {
        ...
        for (int i = offset + 1; i <= batch_size_; ++i) {
            if (i == batch_size_ || token_num + h_context_length_buf_[i] > max_context_token_num_) {
                ...
                star_coder_->contextDecode(nullptr,
                                        ...
                                        const std::vector<int>& lengths)
{
    ...
    if (context_logits_buf_ == nullptr) {
        NcclGuard guard(star_coder_->tensor_para_, stream_, true);
        context_logits_buf_ =
            (float*)allocator_->malloc(sizeof(float) * star_coder_->vocab_size_padded_ * max_context_token_num_);
        const auto tp = star_coder_->tensor_para_.world_size_;
        if (tp > 1) {
            FT_CHECK(star_coder_->vocab_size_padded_ % tp == 0);
            const auto local_vocab_size = star_coder_->vocab_size_padded_ / tp;
            local_context_logits_buf_ =
                (float*)allocator_->malloc(sizeof(float) * local_vocab_size * max_context_token_num_);
        }
    }

    star_coder_->postDecodeEmbedding(context_logits_buf_, local_context_logits_buf_, context_decoder_output, num_token);
    ...
    for (int k = 0; k < indices.size(); ++k) {
        if (output_logits[k]) {
            check_cuda_error(cudaMemcpyAsync(output_logits[k],
                                             logits,
                                             sizeof(float) * star_coder_->vocab_size_ * lengths[k],
                                             cudaMemcpyDefault,
                                             stream_));
        }
        logits += star_coder_->vocab_size_padded_ * lengths[k];
    }
}

template<typename T>
void StarCoderBatch<T>::finish()
{
    ...
    if (rank_ == 0 && star_coder_->ffi_lock_) {
        star_coder_->ffi_lock_(1);
    }
    for (int i = 0; i < batch_size_; ++i) {
...
template<typename T>
void StarCoderBatch<T>::synchronize()
{
    // compact
    int idx = 0;
    for (int i = 0; i < batch_size_; ++i) {
        if (requests_[i]) {
            ...
            check_cuda_error(cudaMemcpyAsync((curandState_t*)topk_curandstate_buf_ + idx,
                                             star_coder_->dynamic_decode_layer_->topk_curandstate_buf() + i,
                                             sizeof(curandState_t),
                                             cudaMemcpyDefault,
                                             stream_));
            check_cuda_error(cudaMemcpyAsync((curandState_t*)topp_curandstate_buf_ + idx,
                                             star_coder_->dynamic_decode_layer_->topp_curandstate_buf() + i,
                                             sizeof(curandState_t),
                                             cudaMemcpyDefault,
                                             stream_));
...
template<typename T>
void StarCoderBatch<T>::setOutputTensors(int max_gen_step)
{
...
template<typename T>
void StarCoderBatch<T>::finishRequest(int index, bool force_end)
{
    if (rank_ == 0) {
    ...
    if (requests_[index]->end_flag || force_end) {
        star_coder_->kv_cache_mgr_->erase(requests_[index]->id);
    }
    else {
        ...
        check_cuda_error(cudaMemcpyAsync(seq.random_state_.data(),
                                         star_coder_->dynamic_decode_layer_->topk_curandstate_buf() + index,
                                         sizeof(curandState_t),
                                         cudaMemcpyDefault,
                                         stream_));
        check_cuda_error(cudaMemcpyAsync(seq.random_state_.data() + sizeof(curandState_t),
                                         star_coder_->dynamic_decode_layer_->topp_curandstate_buf() + index,
                                         sizeof(curandState_t),
                                         cudaMemcpyDefault,
                                         stream_));
        ...
        star_coder_->kv_cache_mgr_->update(cached_seq_[index], stream_);
    }
...
template class StarCoderBatch<half>;
template class StarCoderBatch<float>;
