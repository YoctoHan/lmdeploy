...
#include "src/turbomind/models/star_coder/StarCoderWeight.h"
...

{
    ...
    kv_cache_mgr_ = std::make_unique<StarCoderCacheManager>(num_layer_,
                                                            local_kv_head_num,
                                                            size_per_head_,
                                                            session_len,
                                                            elem_bits,
...
template<typename T>
void StarCoder<T>::embeddingLookup(T* embeddings, const int* token_ids_buf, int batch_size, int step)
{
    TM_LOG_DEBUG(__PRETTY_FUNCTION__);
...
template<typename T>
void StarCoder<T>::contextDecode(T*         deocder_output,
...
template<typename T>
void StarCoder<T>::postDecodeEmbedding(float* logits, float* local_logits, const T* decoder_output, int batch_size)
{
    TM_LOG_DEBUG(__PRETTY_FUNCTION__);
...
template<typename T>
void StarCoder<T>::dynamicDecode(int*            token_ids,
                               bool*           finished,
                               int*            sequence_length,
                               bool*           should_stop,
                               TensorMap*      inputs,
...
template<typename T>
void StarCoder<T>::internalThreadEntry(int device_id)
{
...
template<typename T>
void StarCoder<T>::start()
{
    int device_id = -1;
    check_cuda_error(cudaGetDevice(&device_id));
    internal_thread_ = std::thread(&StarCoder<T>::internalThreadEntry, this, device_id);
}
