...
template<typename T>
class StarCoderDecoderSelfAttentionLayer {
public:
    void freeBuffer();
    void allocateBuffer(size_t batch_size, int key_len, int max_memory_len);

    StarCoderDecoderSelfAttentionLayer(size_t                      head_num,
                                   size_t                      kv_head_num,
    ...
    ~StarCoderDecoderSelfAttentionLayer()
    {
        freeBuffer();
    }

    void forward(TensorMap* output_tensors, const TensorMap* input_tensors, const StarCoderAttentionWeight<T>* weights);
...
private:
    ...
    const StarCoderAttentionParams& params_;
